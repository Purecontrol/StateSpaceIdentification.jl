...
mutable struct ConditionalParticleFilterState
    ...
    function ConditionalParticleFilterState(init_state::GaussianStateStochasticProcess, n_X, n_Y, n_particles, conditional_particle)
        ...
        new(predicted_particles_swarm, predicted_particles_swarm_mean, zeros(Float64, n_X, n_particles), zeros(Float64, n_Y, n_particles), (1/n_particles).*ones(Float64, n_particles), zeros(Int64, n_particles), 0.0, conditional_particle)
...
mutable struct ConditionalParticleFilter <: AbstractFilter

    n_particles::Int64
    init_state_x::GaussianStateStochasticProcess
    filter_state::ConditionalParticleFilterState

    function ConditionalParticleFilter(init_state, n_X, n_Y, n_particles, conditional_particle)

        new(n_particles, init_state, ConditionalParticleFilterState(init_state, n_X, n_Y, n_particles, conditional_particle))

    end

    function ConditionalParticleFilter(model::ForecastingModel; n_particles=30, conditional_particle=nothing)

        new(n_particles, model.current_state, ConditionalParticleFilterState(model.current_state, model.system.n_X, model.system.n_Y, n_particles, conditional_particle))
...
mutable struct CondtionalParticleFilterOutput <: FilterOutput
    ...
    function CondtionalParticleFilterOutput(model::ForecastingModel, y_t, n_particles)
...
function get_filter_output(filter::ConditionalParticleFilter, model, y_t)
    ...
    return CondtionalParticleFilterOutput(model, y_t, filter.n_particles)

end


function filter!(filter_output::CondtionalParticleFilterOutput, sys::StateSpaceSystem, filter::ConditionalParticleFilter, y_t, exogenous_variables, control_variables, parameters)
...
function save_state_in_filter_output!(filter_output::CondtionalParticleFilterOutput, filter_state::ConditionalParticleFilterState, t::Int64)
...
function save_initial_state_in_filter_output!(filter_output::CondtionalParticleFilterOutput, filter_state::ConditionalParticleFilterState)
